...
$function$
;
CREATE OR REPLACE PROCEDURE public.rename_to_john_doe(owner_id_param integer)
    LANGUAGE sql
AS $procedure$
...
CREATE OR REPLACE VIEW vw_pet AS  SELECT pet.id,
                                         pet.type
                                  FROM pet;
CREATE TRIGGER before_owner_update BEFORE UPDATE ON public.owner FOR EACH ROW EXECUTE PROCEDURE before_owner_update_func();
